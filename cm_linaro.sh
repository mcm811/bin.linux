...
arm_eabi_4.7()
{
    ...
    sed -i \
	-e 's/arm-eabi-4.4.3/arm-eabi-4.6/g' \
	-e 's/arm-eabi-4.7.3/arm-eabi-4.6/g' \
	-e 's/arm-eabi-4.7/arm-eabi-4.6/g' \
	device/samsung/galaxys2-common/BoardCommonConfig.mk build/envsetup.sh
    sed -i -e 's/arm-eabi-4.6/arm-eabi-4.7.3/g' \
	device/samsung/galaxys2-common/BoardCommonConfig.mk build/envsetup.sh
...
change_to_changmin()
{
	...
	vendor/cm/config/common.mk buildscripts/build.sh
...
change_to_4.7-jbmali-vsync_sysfs()
{
    ...
    if [ "$(grep vsync_sysfs vendor/cm/config/common.mk|awk '{ print $3 }')" = "-vsync_sysfs" ]; then
	echo "vsync_sysfs-changmin: OK"
	return 0
    fi

    change_to_changmin
    sed -i -e 's/changmin/vsync_sysfs/g' vendor/cm/config/common.mk buildscripts/build.sh
    grep vsync_sysfs vendor/cm/config/common.mk buildscripts/build.sh
    ...
    # hwc: SecHWC.cpp
    sed -i -e 's/#define NO_HW_VSYNC/#define YES_HW_VSYNC/g' \
	hardware/samsung/exynos4/hal/libhwcomposer/SecHWC.cpp

    # kernel: s3cfb_main.c
    sed -i -e 's/#define USE_VSYNC_MODE USE_VSYNC_OFF/#define USE_VSYNC_MODE USE_VSYNC_SYSFS/g' \
...
change_to_4.7-jbmali-vsync_off()
{
    ...
    if [ "$(grep notehwc vendor/cm/config/common.mk|awk '{ print $3 }')" = "-notehwc" ]; then
	echo "notehwc-changmin: OK"
	return 0
    fi

    change_to_changmin
    sed -i -e 's/changmin/notehwc-changmin/g' vendor/cm/config/common.mk buildscripts/build.sh
    ...
    # hwc: SecHWC.cpp
    sed -i -e 's/#define YES_HW_VSYNC/#define NO_HW_VSYNC/g' \
	hardware/samsung/exynos4/hal/libhwcomposer/SecHWC.cpp

    # kernel: s3cfb_main.c
    sed -i -e 's/#define USE_VSYNC_MODE USE_VSYNC_SYSFS/#define USE_VSYNC_MODE USE_VSYNC_OFF/g' \
	kernel/samsung/smdk4210/drivers/video/samsung/s3cfb_main.c

    cd kernel/samsung/smdk4210;SYNC_OFF_DEFCONFIG;cd -
